...
class MagFields:
    ...
    def drift_bounce_orbit(self, X, maginput, alpha=90, R0=1):
        ...
        nposit = npositType()
        lm, lstar, bmin, bmirr, xj, hmin, hmin_lon = [
            ctypes.c_double() for i in range(7)
        ]
    ...
    ### Non-IRBEM methods.
    def bounce_period(
        self, X, maginput, E, Erest=511, R0=1, alpha=90, interpNum=100000
    ):
        """
        ...
        try:
            ...
            endInd = scipy.optimize.brentq(
                fLine["fB"], len(fLine["S"]) / 2, len(fLine["S"]) - 1
            )
        except ValueError as err:
        ...
        if len(fLine["S"]) > interpNum:
            warnings.warn(
                "Warning: interpolating with less data than IRBEM outputs,"
        ...
        if isinstance(E, (np.ndarray, list)):
            self.Tb = np.array(
                [
                    ...
                    * np.sum(
                        np.divide(
                            ds[1:-1],
                            vparalel(Ei, fLine["mirrorB"], dB, Erest=Erest)[1:-1],
                        )
                    )
                    for Ei in E
                ]
            ...
            self.Tb = 2 * np.sum(
                np.divide(
                    ds[1:-1], vparalel(E, fLine["mirrorB"], dB, Erest=Erest)[1:-1]
                )
            )
        return self.Tb
    ...
    def _prepTimeLoc(self, X):
        ...
        assert len(time_key) == 1, (
            f"None or multiple time keys found in dictionary input \n {Xc}"
        )
        time_key = time_key[0]
    ...
    def _prepMagInput(self, inputDict=None):
        ...
        # If model inputs are something else (probably incorrect format)
            ...
            raise TypeError(
                "Model inputs are in an unrecognizable format."
    ...
    def _interpolate_field_line(self, X, maginput, R0=1, alpha=90):
        ...
        fB = scipy.interpolate.interp1d(
            S,
            np.subtract(
                out["blocal"][: out["Nposit"]],
                inputblocal / np.sin(np.deg2rad(alpha)) ** 2,
            ),
            kind="cubic",
        )
...
class Coords:
    ...
    def _cTimes(self, times):
        ...
        if isinstance(times[0], str):
            ...
            raise ValueError(
                "Unknown time format. Valid formats: ISO "
        ...
        for nT in range(N):  # Populate C arrays
            ...
            ut[nT] = ctypes.c_double(
                3600 * t[nT].hour + 60 * t[nT].minute + t[nT].second
            )
    ...
    def _coordSys(self, coordSystem):
        ...
        if isinstance(coordSystem, str):
            assert coordSystem.upper() in lookupTable.keys(), (
                "ERROR: Unknown"
...
def _load_shared_object(path=None):
    ...
    if path is None:
        ...
        matched_object_files = pathlib.Path(__file__).parent.parent / obj_name
        path = matched_object_files
    ...
    try:
        ...
        if "Try using the full path with constructor syntax." in str(err):
            raise OSError(
                f"Could not load the IRBEM shared object file in {path}"
            ) from err
        else:
            raise
...
"""
beta = lambda Ek, Erest=511: np.sqrt(
    1 - ((Ek / Erest) + 1) ** (-2)
)  # Ek,a dErest must be in keV
gamma = lambda Ek, Erest=511: np.sqrt(1 - beta(Ek, Erest=511) ** 2) ** (-1 / 2)
vparalel = (
    lambda Ek, Bm, B, Erest=511: c * beta(Ek, Erest) * np.sqrt(1 - np.abs(B / Bm))
)
